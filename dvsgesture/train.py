...
def main(args):
    ...
    output_dir = os.path.join(
        args.output_dir, f"{args.model}_b{args.batch_size}_T{args.T}"
    )
    ...
    print(
        f"dataset_train:{dataset_train.__len__()}, dataset_test:{dataset_test.__len__()}"
    )
        ...
        if output_dir:
            ...
            if save_max:
                utils.save_on_master(
                    checkpoint, os.path.join(output_dir, "checkpoint_max_test_acc1.pth")
                )
        print(args)
    ...
    if output_dir:
        utils.save_on_master(
            checkpoint, os.path.join(output_dir, f"checkpoint_{epoch}.pth")
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--tb", action="store_true", help="Use TensorBoard to record logs"
    )
